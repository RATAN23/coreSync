...
const Modal = ({dueDate = today(getLocalTimeZone()), title: initialTitle, assignedTo = "", status = "" , handleClose =() =>{}}) => {
  ...
  return (
    ...
    <div className='w-[300px] h-[400px] md:h-[400px] lg:h-1/2 md:w-[400px] lg:w-[500px] rounded-3xl bg-white/90 flex flex-col justify-around'>
       <h1 className='flex justify-center items-center pt-2 text-3xl font-bold'>Task</h1>
      ...
      <div className='relative p-3 flex items-center'>
        <DatePicker 
          label="Due Date"  
          minValue={(dueDate) != today(getLocalTimeZone()) ? (dueDate) : today(getLocalTimeZone())} 
        ...
        <div className='relative ml-auto' ref={statusRef}>
          ...
          >
            <span className='text-black'> {selectedStatus !== "" ? selectedStatus : (status !== "" ? status : "Status")}</span>
            <svg className="w-4 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
      ...
      <div className='relative p-3 flex items-center'>
        <div className='relative ml-auto' ref={namesRef}>
          ...
          >
            <span className='text-black'>{selectedName !== "" ? selectedName :( assignedTo !== "" ? assignedTo : "Assigned to")}</span>
            <svg className="w-4 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
              <path stroke="gray" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="m1 1 4 4 4-4"></path>
            </svg>
          </button>
          {dropDownNames && (
      ...
      <div className='p-4 flex gap-3'>
        <button className='w-20 h-10 rounded-2xl bg-blue-600 text-white' >Save</button>
        <button className='w-20 h-10 rounded-2xl bg-blue-600 text-white' onClick={handleClose}>Cancel</button>
      </div>
    </div>
    </div>
  );
